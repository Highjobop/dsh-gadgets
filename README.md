...
### 📦 dsh-tidy —— 对话整理
- **消息折叠**：对话区左上角按钮切换「已折叠 / 全放开」——折叠时每个回合只保留最后一条回答，中间的思考、工具调用、中间输出全部隐藏
- **导航条**：右侧短横杠节点（每条提问一个），悬停显示前几个字、点击跳转、自动高亮，可滚轮滑动
- **自动加载历史**：打开会话自动点「加载更早」直到加载完（上限 20 次 / 导航容量 100 条）
- 折叠模式 localStorage 持久化，默认全放开
